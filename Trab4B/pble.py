...
@st.cache_data
def carregar_dados(arquivo, tipo='csv', **kwargs):
...
@st.cache_data
def processar_dados(df_pble, df_dtb, df_mun):
    # Filtrar colunas relevantes
    dfFilter = df_pble[["sigla_uf", "id_municipio", "rede", "id_escola", "empresa", "tecnologia", "conexao"]]
    dfFilter['conexao'] = dfFilter['conexao'].str.lower().str.replace(' mbps', '').astype(float)
...
def criar_grafico(tipo, df, **kwargs):
    if tipo == 'bar':
...
df_pble = carregar_dados('databases/pble.csv', encoding='latin-1')
df_dtb = carregar_dados('databases/RELATORIO_DTB_BRASIL_MUNICIPIO.xlsx', tipo='excel')
df_mun = carregar_dados('databases/municipios.csv', encoding='latin-1')

dfFilter = processar_dados(df_pble, df_dtb, df_mun)
...
fig1 = criar_grafico('bar', tecnologia_count, x='Tecnologia', y='Contagem', title='Distribuição de Tecnologia', labels={'Contagem': 'Número de Escolas'})
...
fig2 = criar_grafico('pie', rede_count, values='Contagem', names='Rede', title='Distribuição por Entidade')
...
fig3 = criar_grafico('bar', velocidade_tecnologia, x='tecnologia', y='conexao', title='Velocidade Média de Conexão por Tecnologia', labels={'tecnologia': 'Tecnologia', 'conexao': 'Velocidade (Mbps)'})
...
fig4 = criar_grafico('bar', velocidade_estado, x='sigla_uf', y='conexao', title='Velocidade Média de Conexão por Estado', labels={'conexao': 'Velocidade (Mbps)', 'sigla_uf': 'UF'}, color='cor', color_discrete_map=color_discrete_map)
...
# Algoritmo de Predição - 4° Bimestre - Trabalho
...
st.write("O algoritmo Random Forest (Floresta Aleatória em português) é um algoritmo de aprendizado de máquina utilizado para realizar predições. Resumidamente, o algoritmo cria de forma aleatória várias Árvores de Decisão (Decision Trees) e combina o resultado de todas elas para chegar no resultado final.")
...
st.write("https://www.ibm.com/br-pt/topics/logistic-regression")
st.write("https://icmcjunior.com.br/random-forest/#:~:text=O%20que%20%C3%A9%20Random%20Forest,para%20chegar%20no%20resultado%20final.")
